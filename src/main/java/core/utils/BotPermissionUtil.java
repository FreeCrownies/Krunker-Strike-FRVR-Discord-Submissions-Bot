...
public class BotPermissionUtil {
    ...
    public static EmbedBuilder getUserAndBotPermissionMissingEmbed(Locale locale, GuildChannel channel, Member member,
                                                                   Permission[] userGuildPermissions, Function<Member, Boolean> customUserGuildPermissions,
                                                                   Permission[] userChannelPermissions, Permission[] botGuildPermissions,
                                                                   Permission[] botChannelPermissions
    ) {
        ...
        if (customUserGuildPermissions.apply(member)) userPermission = new ArrayList<>();
    ...
    public static boolean canReadHistory(GuildMessageChannel channel, Permission... permissions) {
        return channel.getGuild().getSelfMember().hasPermission(channel, Permission.MESSAGE_HISTORY) &&
                can((GuildChannel) channel, permissions);
    }

    public static boolean canWrite(GuildMessageChannel channel, Permission... permissions) {
        return channel.getGuild().getSelfMember().hasPermission(channel, Permission.MESSAGE_SEND) &&
                can((GuildChannel) channel, permissions);
    }

    public static boolean canWrite(Member member, GuildMessageChannel channel, Permission... permissions) {
        return member.hasPermission(channel, Permission.MESSAGE_SEND) &&
                can(member, (GuildChannel) channel, permissions);
    }

    public static boolean canWriteEmbed(GuildMessageChannel channel, Permission... permissions) {
    ...
    public static boolean canManage(Guild guild, Optional<Category> category, Permission permission) {
        if (guild.getSelfMember().hasPermission(Permission.ADMINISTRATOR)) {
            return true;
        }

        return permission.getOffset() != Permission.MANAGE_PERMISSIONS.getOffset() &&
                (category.isEmpty() || guild.getSelfMember().hasPermission(category.get(), permission)) &&
                guild.getSelfMember().hasPermission(permission);
    }

    public static boolean canManage(Guild guild, Category category, Permission permission) {
        if (guild.getSelfMember().hasPermission(Permission.ADMINISTRATOR)) {
            return true;
        }

        return permission.getOffset() != Permission.MANAGE_PERMISSIONS.getOffset() &&
                (category == null || guild.getSelfMember().hasPermission(category, permission)) &&
                guild.getSelfMember().hasPermission(permission);
    }
    ...
    public static boolean canUseExternalEmojisInInteraction(GuildMessageChannel channel) {
    ...
    public static boolean channelIsPublic(GuildMessageChannel channel) {
        Guild guild = channel.getGuild();
        MemberCacheController.getInstance().loadMembersFull(guild).join();
    ...
    public static <T extends ICategorizableChannel> ChannelAction<T> copyPermissions(ICategorizableChannel parentChannel, ChannelAction<T> channelAction) {
    ...
    public static <T extends ICategorizableChannel> ChannelAction<T> addPermission(ICategorizableChannel parentChannel, ChannelAction<T> channelAction,
    ...
    private static <T extends ICategorizableChannel> ChannelAction<T> addPermission(ICategorizableChannel parentChannel, ChannelAction<T> channelAction,
    ...
    public static <T extends GuildChannel> ChannelAction<T> addPermission(Guild guild, Optional<Category> category, ChannelAction<T> channelAction,
                                                                          IPermissionHolder permissionHolder, boolean allow, Permission... permissions
    ) {
        return addPermission(guild, category, channelAction, category.map(categoryPresent -> categoryPresent.getPermissionOverride(permissionHolder)).orElse(null), allow,
                permissionHolder instanceof Member, permissionHolder.getIdLong(), permissions);
    }
    ...
    public static <T extends GuildChannel> ChannelAction<T> addPermission(Guild guild, Optional<Category> category, ChannelAction<T> channelAction,
        ...
        return addPermission(guild, category, channelAction, permissionOverride, allow,
                permissionOverride.isMemberOverride(), permissionOverride.getIdLong(), permissions);
    }

    private static <T extends GuildChannel> ChannelAction<T> addPermission(Guild guild, Optional<Category> category, ChannelAction<T> channelAction,
                                                                           PermissionOverride permissionOverride, boolean allow, boolean memberOverride,
                                                                           long id, Permission... permissions
    ) {
        long allowRaw = 0L;
        ...
        List<Permission> allowList = Permission.getPermissions(allowRaw).stream()
                .filter(permission -> BotPermissionUtil.canManage(guild, category, permission))
                .collect(Collectors.toList());
    ...
    public static TextChannelManager addPermission(Guild guild, Optional<Category> category, TextChannelManager channelManager,
                                                   IPermissionHolder permissionHolder, boolean allow, Permission... permissions
    ) {
        return addPermission(guild, category, channelManager, category.map(categoryPresent -> categoryPresent.getPermissionOverride(permissionHolder)).orElse(null), allow,
                permissionHolder instanceof Member, permissionHolder.getIdLong(), permissions);
    }
    ...
    public static TextChannelManager addPermission(Guild guild, Optional<Category> category, TextChannelManager channelManager,
        ...
        return addPermission(guild, category, channelManager, permissionOverride, allow,
                permissionOverride.isMemberOverride(), permissionOverride.getIdLong(), permissions);
    }

    private static TextChannelManager addPermission(Guild guild, Optional<Category> category, TextChannelManager channelManager,
                                                    PermissionOverride permissionOverride, boolean allow, boolean memberOverride,
                                                    long id, Permission... permissions) {
        long allowRaw = 0L;
        long denyRaw = 0L;
    ...
    public static <T extends ICategorizableChannel> ChannelAction<T> clearPermissionOverrides(ChannelAction<T> channelAction) {

...
public class ComponentInteractionResponse extends InteractionResponse {
    ...
    @Override
    public RestAction<Message> editComponents(Collection<ActionRow> actionRows) {
        if (!event.isAcknowledged()) {
            Message message = event.getMessage();
            return event.editComponents(actionRows)
                    .map(h -> message);
        } else {
            return event.getHook().editOriginalComponents(actionRows);
        }
    }

...
public class ShardManager {
    ...
    public static synchronized void increaseGlobalErrorCounter() {
        ...
        if (globalErrors >= 6) {
            System.err.println("EXIT - Too many shard errors");
            System.exit(6);
        }
    }
    ...
    private static void startJDAPoller() {
        // CHANGED 10 sec -> 10 Min

        MainScheduler.poll(10, ChronoUnit.MINUTES, "api_poller", () -> {
            try {
    ...
    public static Optional<String> getGuildName(long guildId) {
        return getLocalGuildById(guildId).map(Guild::getName);
    }
    ...
    public static Optional<Emoji> getLocalEmoteById(long emoteId) {
        for (JDA jda : getConnectedLocalJDAs()) {
            Optional<Emoji> emoteOptional = Optional.ofNullable(jda.getEmojiById(emoteId));
    ...
    public static Optional<String> getEmoteById(long emojiId) {
        return getLocalEmoteById(emojiId).map(Emoji::getFormatted);
    }
